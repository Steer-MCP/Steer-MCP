...
- **Relationship Management** (read & write) — `relationship`, `queue`, `profile`
...
```json
{
  ...
  "app": {
    ...
    "week":   { "remaining": 90, "queue_count": 3, "reset_at": "2026-04-27T00:00:00.000Z" }
  }
}
```
...
Request public data to be found for a person of your choice. Useful if they cannot be found through search. Requires at least one of `linkedin` (URL or bare handle), `refid`, or `relationship_id`. Optional seed fields: `firstName`, `lastName`, `location`, `jobTitle`, `company`, `industry`.
...
Requires `firstName` and `lastName`. Pass `linkedin` (URL or handle) or `refid`. Pass `noQueue: true` when bulk-adding to skip the weekly-refresh queue.
...
- `queued` — true if the relationship was added to the weekly-refresh queue
...
Manage the automatic research queue of people in your network.

#### `action: "view"`

Returns the list of people currently queued.
...
Requires either `refid` or `relationship_id`. Reduces total weekly usage per person.
...
| "Who is in my refresh queue?" | `queue view` |
| "Add David Kim to the refresh queue" | `person search` → `queue add` |
